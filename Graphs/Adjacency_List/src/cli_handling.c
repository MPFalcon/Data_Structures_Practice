...
static int is_negative(const char * num, void ** end_char)
{
    int sign = POS;

    if (0 > strtol(num, (char **)end_char, DEFAULT_BASE))
    {
        sign = NEG;
    }
...
int handle_cli(int argc, char ** argv, uint64_t * requested_size)
{
    ...
    if (1 == argc)
    {
        print_error("Command should be ran like this: 'adjacency_list -s 10'",
                    __func__);
    ...
    switch (opt)
    {
        case 's':
            ...
            *requested_size = strtoul(optarg, &restricted_char, DEFAULT_BASE);

            if (('\0' != *restricted_char) || (errno == ERANGE))
            {
            ...
            if (NEG == is_negative(optarg, (void **)&restricted_char))
            {
                print_error("Size must be a positive integer", __func__);
